...
import fs from 'fs';
import pathLib from '../lib/path.mjs';
const { resolveSafe } = pathLib;

export function createFileApiPlugin({ projectRoot }) {
  return {
    name: 'vuevn-file-api',
    configureServer(server) {
      ...
      server.middlewares.use('/api/files', async (req, res, next) => {
        ...
        try {
          ...
          const targetPath = resolveSafe(projectRoot, dirParam);
          if (!targetPath) return sendJson(res, { error: 'Access denied' }, 403);
      ...
      server.middlewares.use('/api/file', async (req, res, next) => {
        ...
        try {
          ...
          if (!filePath) return sendJson(res, { error: 'Path parameter required' }, 400);
          const targetPath = resolveSafe(projectRoot, filePath);
          if (!targetPath) return sendJson(res, { error: 'Access denied' }, 403);
          if (!fs.existsSync(targetPath)) return sendJson(res, { error: 'File not found' }, 404);
      ...
      server.middlewares.use('/api/create', async (req, res, next) => {
        ...
        try {
          ...
          const targetPath = resolveSafe(projectRoot, relPath);
          if (!targetPath) return sendJson(res, { error: 'Access denied' }, 403);
          const dir = path.dirname(targetPath);
          if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
          if (type === 'directory') {
      ...
      server.middlewares.use('/api/rename', async (req, res, next) => {
        ...
        try {
          ...
          const src = resolveSafe(projectRoot, oldPath);
          const dst = resolveSafe(projectRoot, newPath);
          if (!src || !dst) return sendJson(res, { error: 'Access denied' }, 403);
      ...
      function parseMultipart(buffer, contentType) {
        ...
        while (start !== -1) {
          ...
          const end = next !== -1 ? next - 2 : (endBoundary !== -1 ? endBoundary - 2 : -1);
          if (end === -1) break;
      ...
      server.middlewares.use('/api/assets/upload', async (req, res, next) => {
        ...
        try {
          ...
          if (/multipart\/form-data/i.test(ctype)) {
            ...
            const targetDir = dest ? resolveSafe(projectRoot, dest) : resolveSafe(projectRoot, path.join('global', category));
            if (!targetDir) return sendJson(res, { error: 'Access denied' }, 403);
            const targetPath = resolveSafe(targetDir, file.filename || 'file');
            if (!targetPath) return sendJson(res, { error: 'Access denied' }, 403);
            ensureDir(targetDir);
          ...
          const targetPath = resolveSafe(projectRoot, assetPath);
          if (!targetPath) return sendJson(res, { error: 'Access denied' }, 403);
      ...
      server.middlewares.use('/api/assets/list', async (req, res, next) => {
        ...
        try {
          ...
          function walk(relDir) {
            const absDir = resolveSafe(projectRoot, relDir);
            if (!absDir) return;
...
export default { createFileApiPlugin };
